...
export const FilterSamplePage = memo(() => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      // 格式化时间
      const formattedStartTime = formData.startTime ? format(formData.startTime, "yyyy-MM-dd HH:mm:ss") : "";
      const formattedEndTime = formData.endTime ? format(formData.endTime, "yyyy-MM-dd HH:mm:ss") : "";
      ...
      console.log('准备提交的压滤样数据:', submitData);

      // 暂时使用前端状态管理，不连接数据库
      // TODO: 后续连接数据库时取消注释以下代码
      /*
      const response = await fetch('/api/filter-sample', {
        method: 'POST',
        headers: {
...
FilterSamplePage.displayName = "FilterSamplePage";
