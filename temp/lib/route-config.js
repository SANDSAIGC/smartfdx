...
exports.RouteManager = exports.ROUTE_CONFIG = exports.PageType = exports.AuthStrategy = void 0;
// 身份验证策略枚举
var AuthStrategy;
(function (AuthStrategy) {
    AuthStrategy["NONE"] = "none";
    AuthStrategy["SIMPLE"] = "simple";
    AuthStrategy["SUPABASE"] = "supabase";
    AuthStrategy["ADMIN"] = "admin"; // 管理员认证
...
(function (PageType) {
    ...
    PageType["AUTH"] = "auth";
    PageType["WORKSPACE"] = "workspace";
    PageType["ADMIN"] = "admin";
    PageType["SAMPLE"] = "sample";
    PageType["SYSTEM"] = "system"; // 系统页面
...
class RouteManager {
    ...
    static getRouteByPath(path) {
        return Object.values(exports.ROUTE_CONFIG).find(route => route.path === path);
    }
    /**
     * 根据工作页面名称获取路由配置
     */
    static getRouteByWorkspaceName(workspaceName) {
        return Object.values(exports.ROUTE_CONFIG).find(route => route.workspaceName === workspaceName);
    }
    /**
     * 获取指定身份验证策略的所有路由
     */
    static getRoutesByAuthStrategy(authStrategy) {
        return Object.values(exports.ROUTE_CONFIG).filter(route => route.authStrategy === authStrategy && route.isActive);
    }
    /**
     * 获取指定页面类型的所有路由
     */
    static getRoutesByPageType(pageType) {
        return Object.values(exports.ROUTE_CONFIG).filter(route => route.pageType === pageType && route.isActive);
    }
    /**
     * 检查路径是否需要身份验证
     */
     ...
     * 获取中间件应该排除的路径列表
     */
    static getMiddlewareExcludedPaths() {
     ...
     * 根据用户工作页面获取重定向URL
     */
    static getRedirectUrlByWorkspace(workspaceName) {
    ...
    static getNavigationMenu() {
        const activeRoutes = Object.values(exports.ROUTE_CONFIG).filter(route => route.isActive);
        const menuData = {};
     ...
     * 验证用户是否有权限访问指定路由
     */
    static hasPermission(path, userRole) {
        const route = this.getRouteByPath(path);
        if (!route)
            return false;
        // 如果路由没有角色要求，允许访问
